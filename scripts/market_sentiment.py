...
def _yahoo_get(symbol, period="5d"):
    """Fetch quote from Yahoo Finance direct API with retry."""
        ...
        try:
            ...
            if result and result[0]["indicators"]["quote"][0]["close"]:
                ...
                if len(valid_close) >= 2:
                    # Use actual prev_close from data
                    return {
                        "close": valid_close[-1],
                    ...
                    # Weekend/single data point - return with 0 change
                    return {
                        "close": valid_close[-1],
                        "open": valid_open[-1] if valid_open else valid_close[-1],
...
def get_market_overview():
    ...
    try:
        with OpenQuoteContext() as ctx:
            ret, data = ctx.get_stock_quote(["US.DJI", "US.IXIC", "US.SPX"])
...
def get_vix():
    ...
    try:
        with OpenQuoteContext() as ctx:
            ret, data = ctx.get_stock_quote(["US.VIX"])
            if ret == RET_OK and data is not None and len(data) > 0:
                row = data.iloc[0]
